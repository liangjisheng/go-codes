...
func TestUpdateSave(t *testing.T) {
	...
	user.Age = 100
	db.Save(&user)
	// UPDATE users SET name='ljs1', age=100, birthday='2016-01-01', updated_at = '2013-11-17 21:34:10' WHERE id=111;
}
...
func TestUpdateSpecify(t *testing.T) {
	...
	//db.Model(&user).Select("*").Update(User{Name: "ljs", Role: "admin", Age: 0})

	// Select 除 Role 外的所有字段（包括零值字段的所有字段）
	//db.Model(&user).Select("*").Omit("Role").Update(User{Name: "ljs", Role: "admin", Age: 0})
}
...
func TestBatchUpdate(t *testing.T) {
	...
	err := db.Model(&User{}).Update("name", "ljs").Error // gorm.ErrMissingWhereClause
	if err != nil {
		t.Error(err)
		return
	}

	db.Model(&User{}).Where("1 = 1").Update("name", "ljs")
	// UPDATE users SET `name` = "ljs" WHERE 1=1

	db.Exec("UPDATE users SET name = ?", "ljs")
	// UPDATE users SET name = "ljs"

	db.Session(&gorm.Session{AllowGlobalUpdate: true}).Model(&User{}).Update("name", "ljs")
	// UPDATE users SET `name` = "ljs"
...
func TestUpdateSubSelect(t *testing.T) {
	...
	db.Table("users as u").Where("name = ?", "ljs").
...
func TestUpdateReturn(t *testing.T) {
	...
	// users => []User{{ID: 1, Name: "ljs", Role: "admin", Salary: 100}, {ID: 2, Name: "ljs1", Role: "admin", Salary: 1000}}
	t.Logf("%+v", users)
	...
	// users => []User{{ID: 0, Name: "ljs", Role: "", Salary: 100}, {ID: 0, Name: "ljs1", Role: "", Salary: 1000}}
}
