...
func TestCreateUserMap(t *testing.T) {
	...
	Instance().db.Model(&User{}).Create(map[string]interface{}{
		"name": "ljs", "age": 18, "created_at": time.Now().Unix(),
	})

	// batch insert from `[]map[string]interface{}{}`
	Instance().db.Model(&User{}).Create([]map[string]interface{}{
		{"Name": "ljs1", "Age": 18, "created_at": time.Now().Unix()},
		{"Name": "ljs2", "Age": 20, "created_at": time.Now().Unix()},
	})
}
...
func TestCreateUserSpecifyField(t *testing.T) {
	...
	//INSERT INTO `users` (`name`,`age`) VALUES ("ljs", 20);
}
