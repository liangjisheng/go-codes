...
func TestSelectWhere(t *testing.T) {
	...
	// SELECT * FROM users WHERE name = 'ljs' ORDER BY id LIMIT 1;

	// 获取全部匹配的记录
	db.Where("name <> ?", "ljs").Find(&users)
	// SELECT * FROM users WHERE name <> 'ljs';

	// IN
	db.Where("name IN ?", []string{"ljs", "ljs1"}).Find(&users)
	// SELECT * FROM users WHERE name IN ('ljs','ljs1');

	// LIKE
	db.Where("name LIKE ?", "%ljs%").Find(&users)
	// SELECT * FROM users WHERE name LIKE '%ljs%';

	// AND
	db.Where("name = ? AND age >= ?", "ljs", 10).Find(&users)
	// SELECT * FROM users WHERE name = 'ljs' AND age >= 10;
...
func TestSelectWhereStruct(t *testing.T) {
	...
	// Struct
	db.Where(&User{Name: "ljs", Age: 20}).First(&user)
	// SELECT * FROM users WHERE name = "ljs" AND age = 20 ORDER BY id LIMIT 1;
	t.Logf("%+v", user)

	// Map
	db.Where(map[string]interface{}{"name": "ljs", "age": 20}).Find(&users)
	// SELECT * FROM users WHERE name = "ljs" AND age = 20;
	t.Logf("%+v", user)
	...
	db.Where(&User{Name: "ljs", Age: 0}).Find(&users)
	// SELECT * FROM users WHERE name = "ljs";

	//如果想要包含零值查询条件，你可以使用 map，其会包含所有 key-value 的查询条件，例如
	db.Where(map[string]interface{}{"Name": "ljs", "Age": 0}).Find(&users)
	// SELECT * FROM users WHERE name = "ljs" AND age = 0;

	//当使用 struct 进行查询时，你可以通过向 Where() 传入 struct 来指定查询条件的字段、值、表名，例如
	db.Where(&User{Name: "ljs"}, "name", "Age").Find(&users)
	// SELECT * FROM users WHERE name = "ljs" AND age = 0;

	db.Where(&User{Name: "ljs"}, "Age").Find(&users)
...
func TestSelectFind(t *testing.T) {
	...
	// Plain SQL
	db.Find(&user, "name = ?", "ljs")
	// SELECT * FROM users WHERE name = "ljs";

	db.Find(&users, "name <> ? AND age > ?", "ljs", 20)
	// SELECT * FROM users WHERE name <> "ljs" AND age > 20;
...
func TestSelectNot(t *testing.T) {
	...
	db.Not("name = ?", "ljs").First(&user)
	// SELECT * FROM users WHERE NOT name = "ljs" ORDER BY id LIMIT 1;

	// Not In
	db.Not(map[string]interface{}{"name": []string{"ljs", "ljs1"}}).Find(&users)
	// SELECT * FROM users WHERE name NOT IN ("ljs", "ljs1");

	// Struct
	db.Not(User{Name: "ljs", Age: 18}).First(&user)
	// SELECT * FROM users WHERE name <> "ljs" AND age <> 18 ORDER BY id LIMIT 1;
...
func TestSelectOr(t *testing.T) {
	...
	// Struct
	db.Where("name = 'ljs'").Or(User{Name: "ljs1", Age: 18}).Find(&users)
	// SELECT * FROM users WHERE name = 'ljs' OR (name = 'ljs1' AND age = 18);

	// Map
	db.Where("name = 'ljs'").Or(map[string]interface{}{"name": "ljs1", "age": 18}).Find(&users)
	// SELECT * FROM users WHERE name = 'ljs' OR (name = 'ljs1' AND age = 18);
}
...
func TestSelectJoin(t *testing.T) {
	...
	var user User
	// 带参数的多表连接
	db.Joins("JOIN emails ON emails.user_id = users.id AND emails.email = ?", "ljs@example.org").
...
func TestAdvancedSelect(t *testing.T) {
	...
	//带多个列的 in 查询
	db.Where("(name, age, role) IN ?", [][]interface{}{{"ljs", 18, "admin"}, {"ljs1", 19, "user"}}).Find(&users)
	// SELECT * FROM users WHERE (name, age, role) IN (("ljs", 18, "admin"), ("ljs1", 19, "user"));

	//GORM 支持 sql.NamedArg 和 map[string]interface{}{} 形式的命名参数，例如
	db.Where("name1 = @name OR name2 = @name", sql.Named("name", "ljs")).Find(&user)
	// SELECT * FROM `users` WHERE name1 = "ljs" OR name2 = "ljs"

	db.Where("name1 = @name OR name2 = @name", map[string]interface{}{"name": "ljs"}).First(&user)
	// SELECT * FROM `users` WHERE name1 = "ljs" OR name2 = "ljs" ORDER BY `users`.`id` LIMIT 1
...
func TestAdvancedFirstOrInit(t *testing.T) {
	...
	// 找到了 `name` = `ljs` 的 user
	db.Where(User{Name: "ljs"}).FirstOrInit(&user)
	// user -> User{ID: 111, Name: "ljs", Age: 18}

	// 找到了 `name` = `ljs` 的 user
	db.FirstOrInit(&user, map[string]interface{}{"name": "ljs"})
	// user -> User{ID: 111, Name: "ljs", Age: 18}
	...
	// 找到了 `name` = `ljs` 的 user，则忽略 Attrs
	db.Where(User{Name: "ljs"}).Attrs(User{Age: 20}).FirstOrInit(&user)
	// SELECT * FROM USERS WHERE name = ljs' ORDER BY id LIMIT 1;
	// user -> User{ID: 111, Name: "ljs", Age: 18}
	...
	// 找到 `name` = `ljs` 的记录，依然会更新 Assign 相关的属性
	db.Where(User{Name: "ljs"}).Assign(User{Age: 200}).FirstOrInit(&user)
	// SELECT * FROM USERS WHERE name = ljs' ORDER BY id LIMIT 1;
	// user -> User{ID: 111, Name: "ljs", Age: 200}
	t.Logf("%+v", user)
}
...
func TestAdvancedFirstOrCreate(t *testing.T) {
	...
	// 找到了 `name` = `ljs` 的 user
	db.Where(User{Name: "ljs"}).FirstOrCreate(&user)
	// user -> User{ID: 111, Name: "ljs", "Age": 18}
	...
	// 找到了 `name` = `ljs` 的 user，则忽略 Attrs
	db.Where(User{Name: "ljs"}).Attrs(User{Age: 20}).FirstOrCreate(&user)
	// SELECT * FROM users WHERE name = 'ljs' ORDER BY id LIMIT 1;
	// user -> User{ID: 111, Name: "ljs", Age: 18}
	...
	// 找到了 `name` = `ljs` 的 user，依然会根据 Assign 更新记录
	db.Where(User{Name: "ljs"}).Assign(User{Age: 20}).FirstOrCreate(&user)
	// SELECT * FROM users WHERE name = 'ljs' ORDER BY id LIMIT 1;
	// UPDATE users SET age=20 WHERE id = 111;
	// user -> User{ID: 111, Name: "ljs", Age: 20}
}
...
func TestSelectCount(t *testing.T) {
	...
	var count int64
	db.Model(&User{}).Where("name = ?", "ljs").Or("name = ?", "ljs1").Count(&count)
	// SELECT count(1) FROM users WHERE name = 'ljs' OR name = 'ljs1'

	db.Model(&User{}).Where("name = ?", "ljs").Count(&count)
	// SELECT count(1) FROM users WHERE name = 'ljs'; (count)
